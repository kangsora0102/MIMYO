...
function MySellBid() {
    ...
    const getBidList = () =>{
        if(user)
            axios.get(`${serverIP.ip}/auction/getSellBidList?nowPage=${nowPage}&state=${searchOption}`,{
                headers:{Authorization:`Bearer ${user.token}`}
            })
            .then(res => {
            ...
            </ul>
            {
                ...
                bid.map(item => {
                    return (<ul className='mypage-bid-list'>
                        ...
                        <li className='message-who' id={`mgx-${item.user.id}`} style={{cursor:'pointer',lineHeight:'80px'}}>
                            {item.user.username}
                        </li>
